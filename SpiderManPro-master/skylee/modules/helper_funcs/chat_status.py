...
def user_change(func):
    ...
    def user_change(bot: Bot, update: Update, *args, **kwargs):
        ...
        if not (member.can_change_info or member.status == "creator") and not user in SUDO_USERS:
            update.effective_message.reply_text("Hmm, Looks Like You Can't Change Chat info That Means You Can't Do This Process !")
            return ""
...
def user_pin(func):
    @wraps(func)
    def user_can_pin(bot: Bot, update: Update, *args, **kwargs):
        user = update.effective_user.id
        member = update.effective_chat.get_member(user)
        if not (member.can_change_info or member.status == "creator") and not user in SUDO_USERS:
            update.effective_message.reply_text("Hmm, Looks Like You Can't Pin In This Chat That Means You Can't Do This Process !")
            return ""
        return func(bot, update, *args, **kwargs)
...
def user_can_promote(func):
    @wraps(func)
    def user_can_promote(bot: Bot, update: Update, *args, **kwargs):
        user = update.effective_user.id
        member = update.effective_chat.get_member(user)
        if not (member.can_promote_members or member.status == "creator") and not user in SUDO_USERS:
            update.effective_message.reply_text("Hmm, Looks Like You Can't Promote/Demote In This Chat That Means You Can't Do This Process !")
            return ""
        return func(bot, update, *args, **kwargs)
    
    return user_can_promote

def user_can_delete(func):
    @wraps(func)
    def user_can_dlt(bot: Bot, update: Update, *args, **kwargs):
        user = update.effective_user.id
        member = update.effective_chat.get_member(user)
